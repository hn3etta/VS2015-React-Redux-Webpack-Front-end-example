...
class OpenCoursesPage extends React.Component {
    ...
    loadOCRefreshes(immtblOpenCoursesList) {
        ...
        let ocRfrshTimeoutObjs = [...this.state.openCoursesRefreshArr],
            ocFound = immtblOpenCoursesList.size > 0;
        ...
        immtblOpenCoursesList
            ...
            .forEach(ocCntr => {
                let oc = ocCntr.get("openCourse");
                // Find exisitng refresh timeout object for this diffCntr
                let fndRfrshObj = ocRfrshTimeoutObjs.find(ocRfrshTOObj => ocRfrshTOObj.id == oc.get("id"));
                // If open course refresh object not found then create a new open course refresh object
                let refreshObj = fndRfrshObj ? fndRfrshObj : Object.assign({}, initOpenCourseTimeoutObj, { id: oc.get("id") });
                     
                if ((typeof refreshObj.timeoutHndl === "undefined" || ocCntr.get("refreshSeconds") != refreshObj.secsRefresh) &&
...
OpenCoursesPage.propTypes = {
    ...
    immtblCoursesList: ImmutablePropTypes.list,
